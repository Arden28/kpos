...
class PaymentsReport extends Component
{
    ...
    public function mount() {
        $this->start_date = today()->subDays(30)->format('d-m-Y');
        $this->end_date = today()->format('d-m-Y');
        $this->payments = '';
        $this->query = null;
    }
    ...
    public function getQuery() {
        ...
        if ($this->payments == 'sale') {
            ...
            $this->query = SaleReturnPayment::query()->where('company_id', $current_company_id)->with('saleReturn');
        } elseif ($this->payments == 'purchase') {
            $this->query = PurchasePayment::query()->where('company_id', $current_company_id)->with('purchase');
        } elseif ($this->payments == 'purchase_return') {
            $this->query = PurchaseReturnPayment::query()->where('company_id', $current_company_id)->with('purchaseReturn');
        } else {
            $this->query = null;
        }
    }
